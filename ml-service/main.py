...
def mock_predict(usage: float, complaints: int, payment_delay: int) -> float:
    base_risk = 0.3  # Increased base risk
    usage_factor = 0.5 if usage < 10 else 0.3 if usage < 20 else 0.1 if usage < 50 else -0.1
    complaints_factor = complaints * 0.2  # Increased complaint impact
...
async def predict_churn(request: ChurnPredictionRequest):
    start_time = time.time()
    try:
        print(f"PREDICT REQUEST: {request.model_dump()}")
...
if __name__ == "__main__":
    import uvicorn
    module = "ml-service.main" if __package__ else "main"
    uvicorn.run(f"{module}:app", host=settings.HOST, port=settings.PORT, reload=settings.DEBUG)
