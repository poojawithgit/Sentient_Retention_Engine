...
export const BrandLogo = ({ className = "", isSidebar = false, hideText = false }) => (
  <div className={`flex ${isSidebar ? 'flex-col items-center justify-center text-center' : 'items-center gap-4'} group cursor-default ${className}`}>
    <div className="relative shrink-0">
      <div className={`${isSidebar ? 'w-14 h-14 rounded-xl mb-4' : 'w-14 h-14 rounded-2xl'} bg-cyber-primary flex items-center justify-center shadow-[0_0_40px_rgba(197,248,42,0.3)] group-hover:shadow-[0_0_60px_rgba(197,248,42,0.5)] transition-all duration-700 overflow-hidden relative border border-white/20`}>
        <Shield size={isSidebar ? 26 : 28} className="text-[#0a110b] relative z-20 group-hover:scale-110 transition-transform duration-500" />
    ...
    {!hideText && (
      <div className={`flex flex-col ${isSidebar ? 'items-center' : 'min-w-0'}`}>
        <div className={`flex items-center ${isSidebar ? 'gap-0 mb-0.5' : 'gap-3 mb-1.5'} leading-none`}>
          <span className={`${isSidebar ? 'text-[8px]' : 'text-[12px]'} font-bold text-cyber-primary uppercase tracking-[0.4em] font-mono opacity-100 drop-shadow-[0_0_8px_rgba(197,248,42,0.4)]`}>Sentient</span>
          {!isSidebar && <div className="h-[1px] w-12 bg-gradient-to-r from-cyber-primary/60 to-transparent" />}
        </div>
        <div className={`flex items-baseline ${isSidebar ? 'gap-0' : 'gap-2'} leading-none`}>
          <span className={`${isSidebar ? 'text-sm' : 'text-4xl'} font-display text-white tracking-[0.2em] uppercase group-hover:text-cyber-primary transition-all duration-500 drop-shadow-sm`}>Retention</span>
          {!isSidebar && (
        ...
        {isSidebar && (
          <span className="text-[8px] font-mono text-zinc-500 uppercase tracking-[0.15em] font-bold mt-1 opacity-60">Engine_v5.0</span>
        )}
      </div>
    )}
  </div>
